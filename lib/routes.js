...
module.exports = function(settings) {
	...
	var playerModel = require("./playermodel.js")(settings);
	var tribeModel = require("./tribemodel.js")(settings);
	var arkserver = require("./server.js")(settings);
	var refreshing = true;
	var cacheInt;
	...
	cacheInt = setInterval(refreshCache, settings.server_settings.cache_refresh);
				...
				getTribeLog: {
					...
					run: function(req, res) {
						if (req.body.id === undefined) {
							...
							tribeModel.getTribeLog(req.body.id, function(d) {
								if (d === undefined) {
									res.statusMessage = "Tribelog Not Found!";
									res.status(400).end();
								} else {
									res.json({
										d: d
